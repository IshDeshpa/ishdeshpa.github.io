...
const numChars = 95;
export class Font {
    constructor(gl, version, size, inverted = false, fontfamily = 'monospace') {
        this.gl = gl;
        this.init(version, size, inverted, fontfamily);
    }
    init(version, size, inverted, fontfamily) {
        ...
        let computeVector = (char, size, charWidth) => { return [0]; };
        let texWidth = 0;
        if (version == 1) {
        ...
        let maxes = new Array(numActualComponents);
        maxes.fill(0, 0, numActualComponents);
        for (let i = 0; i < printable.length; i++) {
            ctx.fillText(printable[i], i * charWidth, 0);
            const weightVec = computeVector(String.fromCharCode(32 + i), size, charWidth);
            // if(this.inverted){
        ...
        for (let i = 0; i < numChars; i++) {
            for (let j = 0; j < numActualComponents; j++) {
                if (!this.inverted) {
                    this.weights[i * numComponents + j] = Math.pow((1 - (this.weights[i * numComponents + j] / maxes[j])), 4);
                }
                else {
                    this.weights[i * numComponents + j] = Math.pow(((this.weights[i * numComponents + j] / maxes[j])), 4);
                }
            }
        }
    ...
    compute6DShapeVector(char, size, charWidth) {
    ...
    compute2DShapeVector(char, size, charWidth) {
        const canvas = document.getElementById("charcanvas");
        const ctx = canvas.getContext('2d');
        canvas.width = charWidth;
    ...
    measureLuminance(char, size, charWidth) {
        const canvas = document.getElementById("charcanvas");
        const ctx = canvas.getContext('2d');
        canvas.width = charWidth;
        canvas.height = Math.ceil(size * 1.5);
    ...
    setFont(fontfamily) {
        return __awaiter(this, void 0, void 0, function* () {
            yield document.fonts.load(`${this.size}px "${fontfamily}"`);
