...
export abstract class Animation{
  ...
  protected bg: Vec4 = new Vec4([1.0, 1.0, 1.0, 1.0]);
  ...
  constructor(gl: WebGLRenderingContext, extVAO: OES_vertex_array_object, timeout: number, chain: FXProgram[]){
    this.gl = gl;
    this.extVAO = extVAO;
  ...
  public abstract onKeydown(key: KeyboardEvent): void;
}
...
export class ShapeAnimation extends Animation{
  ...
  constructor(gl: WebGLRenderingContext, extVAO: OES_vertex_array_object, chain: FXProgram[]){
  ...
  public init(): void {
    ...
    this.program = new GeometryProgram(
      this.gl,
      this.extVAO,
      this.bg,
      vsText,
...
interface PlanetConfig {
  ...
  orbitRadius: number;
  color: Vec3;
  orbitSpeed: number;  // relative, based loosely on Kepler's third law
  axialTilt: Vec3;
}
...
export class SpaceAnimation extends Animation{
  private sun!: SphereGeometry;
  private planets!: SphereGeometry[];
  ...
  public init(): void {
    this.sun = new SphereGeometry(
      1, 32, 32,
      new Vec3([0, 0, 0]),
      Quat.identity.copy(),
      new Vec3([1.0, 0.85, 0.1])
    );

    this.planets = PLANETS.map(p => new SphereGeometry(
      p.radius, 32, 32,
      new Vec3([p.orbitRadius, 0, 0]),
      Quat.identity.copy(),
      p.color
    ));
...
export class VideoAnimation extends Animation{
  ...
  private videoTexture: WebGLTexture = -1;
  private videoUpload: HTMLInputElement;

  constructor(gl: WebGLRenderingContext, extVAO: OES_vertex_array_object, chain: FXProgram[]){
    super(gl, extVAO, 1, chain);
    this.video = document.getElementById("video") as HTMLVideoElement;
    this.videoUpload = document.getElementById("upload") as HTMLInputElement;
...
export class FXChain{
  public source: RenderTarget;
  private intermediates: RenderTarget[];
  private chain: FXProgram[];

  constructor(gl: WebGLRenderingContext, extVAO: OES_vertex_array_object, source: RenderTarget, chain: FXProgram[]){
    this.source = source;
    this.chain = chain;
    ...
    this.chain.push(
      new FXProgram(
        gl, extVAO, defaultfxVSText, defaultfxFSText
      )
    );
  ...
  public getAllAsciiFX(): AsciiProgram[]{
    let asciis = [];
